...
//			NSLog(@"[AUTH] Request %@ failed or was cancelled while waiting to access credentials",self);
		...
		if ([self useSessionPersistence]) {
			...
			if (credentials && [self applyCredentials:[credentials objectForKey:@"Credentials"]]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
//				NSLog(@"[A/UTH] Request %@ will reuse cached credentials from the session (%@)",self,[credentials objectForKey:@"AuthenticationScheme"]);
				#endif
...
//			NSLog(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - will retry with a new connection", [[self connectionInfo] objectForKey:@"id"]);
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
//		NSLog(@"[CONNEC//TION] Request attempted to use connection #%@, but it has been closed - we have already retried with a new connection, so we must give up", [[self connectionInfo] objectForKey:@"id"]);
	#endif
	return NO;
}
...
#if DEBUG_PERSISTENT_CONNECTIONS
//			NSLog(@"[CONN/ECTION] Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] intValue]);
#endif
